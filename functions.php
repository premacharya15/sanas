...
function move_to_guest_list() {
    ...
    foreach ($guest_ids as $guest_id) {
        ...
        if ($guest) {
            // Check if email already exists for the specific event
            $email_exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $guest_info_table WHERE guest_event_id = %d AND guest_email = %s AND guest_id != %d",
                $event_id, $guest['guest_email'], $guest_id
            ));
...
function send_contact_email() {
    ...
    $user_mail_sent = wp_mail($email, $user_subject, $user_body, $headers);
...
// Function to add a to-do item and handle "Save and Add More"
add_action('wp_ajax_add_todo_item_and_add_more', 'add_todo_item_and_add_more');
function add_todo_item_and_add_more() {
    global $wpdb;
    $current_user_id = get_current_user_id();
    ...
    if ($inserted) {
        // Fetch the inserted to-do item
        $todo_id = $wpdb->insert_id;
        $todo_item = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}todo_list WHERE id = %d",
            $todo_id
        ), ARRAY_A);

        // Generate the HTML for the new to-do item row
        ob_start();
        ?>
